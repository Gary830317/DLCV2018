...
start = 700
true_label = np.array(fulllength_valid_action_labels[0][start:start+300]).astype(int).reshape(1,-1)
with open('OP01-R03-BaconAndEggs.txt', 'r') as file:
    predict_label = np.array([line.rstrip() for line in file][start:start+300]).astype(int).reshape(1,-1)


# In[282]:


true_label = np.repeat(true_label,4,axis=1)
true_label = np.repeat(true_label,20,axis=0)
predict_label = np.repeat(predict_label,4,axis=1)
predict_label = np.repeat(predict_label,20,axis=0)
...
plt.axis('off')
plt.imshow(true_label, cmap="rainbow")
plt.savefig('true_label.jpg', bbox_inches='tight')
...
plt.figure(figsize=(20,500))
plt.axis('off')
plt.imshow(predict_label, cmap="rainbow")
plt.savefig('predict_label.jpg', bbox_inches='tight')


# In[250]:


pick = list(range(0,300,33))

fulllength_valid_frame_dir = './HW5_data/FullLengthVideos/videos/valid/OP01-R03-BaconAndEggs'
fulllength_valid_frame = [file for file in os.listdir(fulllength_valid_frame_dir) if file.endswith('.jpg')][start:start+300]  
fulllength_valid_frame.sort()
fulllength_valid_frame = np.array(fulllength_valid_frame)[pick]
video_frames = []
for frame in fulllength_valid_frame:
    video_frames.append(imread(os.path.join(fulllength_valid_frame_dir,frame)))
...
def plot(imgs, row, col):
    ...
    for i, img in enumerate(imgs):
        ...
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        plt.imshow(img.reshape(240, 320, 3))
    return img
...
lll=len(pick) 
fig = plot(video_frames,1,lll)
plt.savefig('frames.jpg', bbox_inches='tight')
